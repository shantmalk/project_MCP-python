...
def run(path_wr=''):
    ...
    # Make pivot table
    pd_qsel_pivot_totals = pd.pivot_table(pd_qsel_data, index='mi_type', aggfunc=len, values=['confirm_idc_str'])
    pd_qsel_pivot_totals.columns = ['total_patients']
    pd_qsel_pivot_totals.reset_index(inplace=True)
    
    pd_qsel_pivot_mean = pd.pivot_table(pd_qsel_data, index='mi_type', aggfunc="mean", values=['SumOfmass_mcp_g', 'SumOfmass_mcp_perc'])
    pd_qsel_pivot_mean.reset_index(inplace=True)
    
    pd_qsel_pivot_std = pd.pivot_table(pd_qsel_data, index='mi_type', aggfunc=np.std, values=['SumOfmass_mcp_g', 'SumOfmass_mcp_perc'])
    ...
    pd_qsel_pivot = reduce(lambda left,right: pd.merge(left, right, on=['mi_type'],how='outer',), pd_qsel_pivot_list)   
