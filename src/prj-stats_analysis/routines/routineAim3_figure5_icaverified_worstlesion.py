...
def run(path_wr=''):
    ...
    lib_prj.visualize.print_label('routineAim3')
    
    # ------------------------------- PARSE -------------------------------- #
    
    fpath_json = lib_prj.paths.PATH_TEMPLATE_JSON.format('qsel_mi_versus_nomi')
    fkey_json = 'mi_icalesionverified_unmatched'
    
    pd_qsel_data_ica_verified = lib_prj.parse.qsel_parse(lib_prj.paths.PATH_DB, fpath_json, fkey_json)
    pd_qsel_data_ica_verified = pd_qsel_data_ica_verified.replace(r'^\s*$', '0', regex=True)
    pd_qsel_data_ica_verified['lesion_ica_verified'] = 1
    pd_qsel_data_ica_verified['lesion_cta_verified'] = 0
    
    fpath_json = lib_prj.paths.PATH_TEMPLATE_JSON.format('qsel_mi_versus_nomi')
    fkey_json = 'mi_lesionworst_unmatched'
    
    pd_qsel_data_cta_verified = lib_prj.parse.qsel_parse(lib_prj.paths.PATH_DB, fpath_json, fkey_json)
    pd_qsel_data_cta_verified = pd_qsel_data_cta_verified.replace(r'^\s*$', '0', regex=True)
    pd_qsel_data_cta_verified['lesion_cta_verified'] = 1
    pd_qsel_data_cta_verified['lesion_ica_verified'] = 0
    
    pd_qsel_data = pd.concat([pd_qsel_data_ica_verified, pd_qsel_data_cta_verified])
    ...
    conds = [
        (pd_qsel_data['lesion_ica_verified'] == 1),
        (pd_qsel_data['lesion_cta_verified'] == 1),
        ]
    
    vals = ['ICA Verified', 'CTA Verified']
    pd_qsel_data['lesion_verified_str'] = np.select(conds, vals)
    ...
    anova_grps_b = ['lesion_ica_verified', 'lesion_cta_verified']
    ...
    print(tabulate(pd_qsel_pivot.sort_values(by=['mi_type_str', 'lesion_verified_str']), headers='keys', tablefmt='psql'))
    lib_prj.visualize.table_basic(pd_qsel_pivot.sort_values(by=['mi_type_str', 'lesion_verified_str']), path_analysis_png + 'table5b.png', ['w', 'w', '#f1f1f2', '#f1f1f2',])
    ...
    lib_prj.visualize.table_basic(pd_qsel_pivot, path_analysis_png + 'table5a.png')
    # ------------------------------ VISUALIZE ----------------------------- #
    ...
    args_plotly = {
        'x' : 'lesion_verified_str',
        'points' : 'all',
        'color' : 'lesion_verified_str',
        'labels' : {'lesion_verified_str' : 'Lesion Type'},
        }
